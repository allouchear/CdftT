...
void ComputeCondensedLinearResponse::run()
{
    ...
    // Read compute energy option
    bool computeEnergyWithOtherMethods;
    readComputeEnergyWithOtherMethods(computeEnergyWithOtherMethods);
    ...
    // Check the number of analytic files provided
    if (computeEnergyWithOtherMethods)
    {
        if (analyticFilesNames.size() != 4)
        {
            std::stringstream errorMessage;
            errorMessage << "Error in Job::run_computeCondensedLinearResponse(): the \"AnalyticFiles\" parameter must contain exactly four file names when \"ComputeEnergyWithOtherMethods\" is set to true." << std::endl;
    ...
    else
    {
        ...
        {
            std::stringstream errorMessage;
            errorMessage << "Error in Job::run_computeCondensedLinearResponse(): the \"AnalyticFiles\" parameter must contain exactly one file name when \"ComputeEnergyWithOtherMethods\" is set to false." << std::endl;
            errorMessage << "Please check the documentation and the \"AnalyticFiles\" parameter value in the provided input file.";
    ...
    // Save results
    std::ofstream outputFile(outputPrefix + "eigenvalues.cdftt");
    if (!outputFile)
    {
        std::stringstream errorMessage;
        errorMessage << "Error in Job::run_computeCondensedLinearResponse(): could not open output file " << outputPrefix << "eigenvalues.cdftt for writing." << std::endl;
    ...
    outputFile.open(outputPrefix + "eigenvectors.cdftt");
    if (!outputFile)
    {
        std::stringstream errorMessage;
        errorMessage << "Error in Job::run_computeCondensedLinearResponse(): could not open output file " << outputPrefix << "eigenvectors.cdftt for writing." << std::endl;
