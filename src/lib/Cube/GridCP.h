#ifndef CDFTT_GRIDCP_H_INCLUDED
#define CDFTT_GRIDCP_H_INCLUDED
...
class GridCP
{
    private:
        Domain _domain;
        Structure _str;
        std::vector<std::vector<std::vector<std::vector<double>>>> _V;
        std::vector<std::vector<std::vector<int>>> _volumeNumberOfPoints;
        std::vector<std::vector<std::vector<int>>> _known;
        ...
        CriticalPoint  newCriticalPoint(int i, int j, int k, int numV);
            ...
            //! reset known
            /*! resets the std::vector containing known points to 0s.*/
        void resetKnown();
        ...
        bool nextPointOnGrid(int current[], int next[]);
            ...
            //! Chooses next point
            /*! Same as On Grid but calculates the correction std::vector required t find the true assent trajectory for near grid method*/
        ...
        bool addSurroundingEqualPoints(int current[], std::vector<std::vector<int>>& listOfVisitedPoints);
        ...
        std::vector<std::vector<int>> assentTrajectory(int current[],bool ongrid, bool refine);
        ...
        bool okDomain();
            ...
            //! Init 3D std::vector
            /*! Returns a std::vector of size N1xN2xN3 of zeroes*/
        std::vector<std::vector<std::vector<int>>> get3DIntVector();
        ...
        bool addSurroundingSignPoints(int current[], std::vector<std::vector<int>>& listOfVisitedPoints, double cutoff);
    ...
    public:
        ...
        void buildBasins(const Grid&, PartitionMethod partitionMethod);
        ...
        std::vector<double> computeAIMCharges(const Grid&);
