...
This job computes the electron density of a system and saves it in a ``.cube`` file.
...
.. code-block:: none
    ...
    # Input file name (supported formats: .fchk, .gab, .log, .molden, .wfx)
    ...
    # Output file name prefix (with the example below, the output file for the density of state 1 will be "systemXXX_density_state1.cube")
